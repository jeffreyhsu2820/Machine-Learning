...
# 分別將 train_x、train_y、test_x 讀進來
train_x= readfile(sys.argv[1], False)
with open(sys.argv[2]) as f:
  next(f)
  train_y = np.array([line.strip('\n').split(',')[1] for line in f], dtype = float)

def _train_dev_split(X, Y, valid_ratio):
...
train_x, train_y, val_x, val_y = _train_dev_split(train_x, train_y, 0.25)
...
torch.save(model_best.state_dict(), 'hw3_train.pth')
